...
//    }
    int sum = 0;
    public int totalDepth(Node node){
        if(node == null) return 0;
        else{
            totalDepth(node.smaller);
            sum = sum + node.count;
            totalDepth(node.larger);
        }
        return sum;
    }

    public double averDepth(Node node){
        double averDepth = 0;
        if (node == null) return 0;
        else {
            double total_depth = (double) totalDepth(node);
            averDepth = (total_depth - node.count) / node.count;
            return averDepth;
        }
    }
    ...
    public int depth(Node x){
        ...
        return 1 + Math.max(depth(x.smaller), depth(x.larger));
    }
