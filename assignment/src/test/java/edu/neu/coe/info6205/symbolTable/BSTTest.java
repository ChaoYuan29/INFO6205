...
public class BSTTest {
    ...
    public void testDeleteAndInsert() throws Exception{
        ...
        for(int j = 0; j<80 ; j++) {
            ...
            int n = 0;
            while (n < size) {
                map.put(String.valueOf(random.nextInt(size)), random.nextInt(200));
                n++;
            }
            bst.putAll(map);

            for (int i = 0; i < 1000000; i++) {
                if (randBoolean.nextInt(2) == 0) {
                    bst.put(String.valueOf(randKey.nextInt(size)), randKey.nextInt(200));
                } else {
                    bst.delete(String.valueOf(randKey.nextInt(size)));
                }
            }

            double aver = ((BSTSimple<String, Integer>) bst).averDepth(((BSTSimple<String, Integer>) bst).root);
            System.out.println("N = " + size);
            System.out.println("MaxDepth: " + bst.maxDepth());
            System.out.println("AverageDepth:" + aver);
            size += 10;
        }
    }
